...
def argument_parser():
    ...
    parser.add_argument(
        "--nfft", default=256, type=int, help="FFT length."
    )
    parser.add_argument(
...
def main():
    ...
    freq_resolution = sampling_rate / fft_len #0.5e6 # freq_resolution = sampling_rate / fft_len
    draw_rate = 1
    y_label_skip = 3
    ...
    XX,YY = np.meshgrid(np.linspace(min_freq,max_freq,int((max_freq-min_freq)/(freq_resolution)+1)),np.linspace(db_min, db_max,psd_db_resolution))

    psd_x_edges = XX[0]
    psd_y_edges = YY[:,0]

    mesh_psd = ax_psd.pcolormesh(XX,YY, np.zeros(XX[:-1,:-1].shape), shading="flat")
    max_psd_ln, = ax_psd.plot(X[0],db_min * np.ones(freq_data.shape[1]), color="red", marker=",", linestyle=":", markevery=10, label="max")
    min_psd_ln, = ax_psd.plot(X[0],db_min * np.ones(freq_data.shape[1]), color="pink", marker=",", linestyle=":", markevery=10, label="min")
    mean_psd_ln, = ax_psd.plot(X[0],db_min * np.ones(freq_data.shape[1]), color="cyan", marker="^", markersize=8, fillstyle="none", linestyle=":", markevery=20, label="mean")
    var_psd_ln, = ax_psd.plot(X[0],db_min * np.ones(freq_data.shape[1]), color="magenta", marker="s", markersize=8, fillstyle="none", linestyle=":", markevery=20, label="variance")
    current_psd_ln, = ax_psd.plot(X[0],db_min * np.ones(freq_data.shape[1]), color="red", marker="o", markersize=8, fillstyle="none", linestyle=":", markevery=10, label="current")
    ...
    for _ in range(top_n): 
        ...
        ln, = ax.plot([X[0][0]]*len(Y[:,0]), Y[:,0], color="brown", linestyle=":", alpha=0)
        top_n_lns.append(ln)
    ax.set_xlabel("MHz")
    ax.set_ylabel("Time")
    ...
    title = ax.text(.5, 1.05, '', transform = ax.transAxes, va='center', ha='center')
    psd_title = ax_psd.text(.5, 1.05, '', transform = ax_psd.transAxes, va='center', ha='center')
    title_text = {}
    y_ticks = []
    y_labels = []
    ...
    with open(args.fft_log,"r") as logfile: 
        ...
        for line in loglines:
            ...
            if not waterfall_row or waterfall_row[-1][freq_idx] < line_floats[freq_idx]: 
                ...
                idx = np.array([round((item[freq_idx] - min_freq) / freq_resolution) for item in waterfall_row]).astype(int)
                ...
                freq_data[-1][idx] = [round(item[freq_idx]/freq_resolution)*freq_resolution for item in waterfall_row]
                ...
                data,xedge,yedge = np.histogram2d(freq_data[~np.isnan(freq_data)].flatten(), db_data[~np.isnan(db_data)].flatten(), density=False,  bins=[psd_x_edges, psd_y_edges])#bins=(freq_data.shape[1], psd_db_resolution))#, range=[[0, 400], [db_min, db_max]])
                ...
                top_n_bins = freq_bins[np.argsort(np.nanvar(db_data  - np.nanmin(db_data, axis=0), axis=0))[::-1][:top_n]]
                #print(f"{top_n_bins=}")
               ...
               # fig.canvas.draw()
                ...
                if counter % draw_rate == 0: 
                    ...
                    if plot_snr: 
                        db_norm = ((db_data - np.nanmin(db_data, axis=0))-snr_min) / (snr_max - snr_min)
