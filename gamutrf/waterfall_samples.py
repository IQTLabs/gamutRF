...
FFT_FILENAME_RE = re.compile(r"^.+_([0-9]+)_([0-9]+)points_([0-9]+)Hz_([0-9]+)sps\.(s\d+|raw).*$")
SAMPLE_DTYPES = {
...
def argument_parser():
    ...
    parser.add_argument(
        "--write_samples", default=2048, type=int, help="Number of samples written during scan."
    )
...
def main():
    ...
    X,Y = np.meshgrid(np.linspace(plot_min_freq,plot_max_freq,int((max_freq-min_freq)/freq_resolution+1)), np.linspace(0,total_time,expected_time_bins))
    freq_bin_vals = X[0]
    ...
    title = ax.text(.5, 1.02, '', transform = ax.transAxes, va='center', ha='center')
    ...
    processed_files = []
    while True:
        sample_files = [f for f in sorted(os.listdir(sample_dir)) if f.startswith("sample") and f.endswith(".zst")]
        ...
        for f in sample_files: 
            ...
            if (
                ((((freq_center + (sample_rate/2)) >= min_freq) and ((freq_center + (sample_rate/2)) <= max_freq)) 
                or (((freq_center - (sample_rate/2)) >= min_freq) and ((freq_center - (sample_rate/2)) <= max_freq)))
                and f not in processed_files
            ):
                if not processing_batch or processing_batch[-1]["freq_center"] < freq_center: 
        ...
        if needs_processing: 
            ...
            for file_info in needs_processing[0]:
                ...
                samples = read_samples(filename, sample_dtype, sample_bytes, seek_bytes=0, nfft=nfft, n=n)
                if samples is None: 
                    continue
                freq_bins, t_bins, S = signal.spectrogram(samples, sample_rate, window=signal.hann(int(nfft), sym=True), nperseg=nfft, noverlap=noverlap, detrend='constant', return_onesided=False) 
                freq_bins = np.fft.fftshift(freq_bins)
                #print(f"{freq_bins.shape=}{freq_center + freq_bins=}")

                idx = np.array([round((item - min_freq) / freq_resolution) for item in freq_bins+freq_center]).astype(int)
                # print(f"{idx=}")
                ...
                spec_data[:S_norm.shape[0], idx[np.flatnonzero((idx>=0) & (idx <= max_idx))] ] = S_norm[:,np.flatnonzero((idx>=0) & (idx <= max_idx))]
