...
class ReadRecordingTestCase(unittest.TestCase):
    ...
    def test_read_recording(self):
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            for i, sample_chunk in enumerate(read_recording(recording, sample_rate, sample_dtype, sample_len)):
    ...
    def test_sjip_read_recording(self):
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            for i, sample_chunk in enumerate(read_recording(recording, sample_rate, sample_dtype, sample_len, skip_sample_secs=1.0, max_sample_secs=0.5)):
                self.assertTrue(isinstance(sample_chunk, np.ndarray))
